...
| `SUPABASE_SERVICE_ROLE_KEY` | Build 6 push/cron only | Server-only key used by cron and push delivery; never expose as `NEXT_PUBLIC_` |
...
Build 6 adds database-backed in-app notifications plus optional browser/PWA push for:

- first active broker response on a REQ
- owner connection to a respondent
- REQs entering their final 24 hours
- account approval

Notifications are stored in `public.notifications` with owner-only RLS. Browser push subscriptions are stored in `public.push_subscriptions`; users can manage only their own subscription rows. Push payloads contain only short safe context and never include phone numbers, emails, buyer identity, private notes, or inventory details.

The app never asks for notification permission on page load. Brokers can enable notifications from the post-success card or the profile notifications section. Push delivery uses standard Web Push with VAPID keys. Invalid browser endpoints are removed after a 404/410 delivery response.

The expiring-REQ reminder is generated by the Vercel Cron route:

```text
GET /api/cron/req-expiring
